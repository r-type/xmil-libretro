...
@	.section	.text
	...
	; @ 元々静的関数でインライン展開されるから遅くなる

	.if 0
minclock:		ldr		r1, [r0, #CTCCHS_CMD]
				mov		r2, #0x01000000
				...
				ldr		r2, [r0, #(CTCCHS_COUNT + 3*4)]		; @ 無条件
				tst		r1, #(0x40 << 24)
				beq		1f

				sub		r2, r2, #1
				ldr		r3, [r0, #(CTCCHS_COUNTMAX + 0*4)]
				ldr		r12, [r0, #(CTCCHS_COUNT + 0*4)]
				mla		r2, r3, r2, r12

1:				and		r12, r1, #(0x82 << 0)
				teq		r12, #(0x80 << 0)
				ldreq	r3, [r0, #(CTCCHS_COUNT + 0*4)]
				cmpeq	r3, r2
				movmi	r2, r3

				and		r12, r1, #(0x82 << 8)
				teq		r12, #(0x80 << 8)
				ldreq	r3, [r0, #(CTCCHS_COUNT + 1*4)]
				cmpeq	r3, r2
				movmi	r2, r3

				and		r12, r1, #(0x82 << 16)
				teq		r12, #(0x80 << 16)
				ldreq	r3, [r0, #(CTCCHS_COUNT + 2*4)]
				cmpeq	r3, r2
				movmi	r2, r3
