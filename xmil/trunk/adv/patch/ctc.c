...
static SINT32 minclock(const CTCCH *ch) {
	...
	event = 0x01000000;
	cmd = *(UINT32 *)(ch->s.cmd);
	if ((cmd & (0x82 << 0)) == (0x80 << 0)) {
		clock = ch->s.count[0];
		event = min(event, clock);
	}
	if ((cmd & (0x82 << 8)) == (0x80 << 8)) {
		clock = ch->s.count[1];
		event = min(event, clock);
	}
	if ((cmd & (0x82 << 16)) == (0x80 << 16)) {
		clock = ch->s.count[2];
		event = min(event, clock);
	}
	if ((cmd & (0x82 << 24)) == (0x80 << 24)) {
		clock = ch->s.count[3];
		if (cmd & (0x40 << 24)) {
...
#if 1
extern REG8 __attribute__((long_call)) ctcwork(CTCCH *ch);
#else
extern REG8 ctcwork2(CTCCH *ch);
	CTCCH	ctcch_chk;
...
//								ch->s.basecnt[3], ch->s.cmd[3]));
	...
	if (intr != ctcwork2(&ctcch_chk)) {
		while(1) { }
	}
	if (memcmp(ch, &ctcch_chk, sizeof(ctcch_chk))) {
		while(1) { }
	}
	return(intr);
}
