...
static UINT8 key106[256] = {
			...
			//	 SFT,CTRL, ALT,PAUS,CAPS,KANA,    ,    		; 0x10
				0x70,0x74,0x73,0x60,0x81,0x82,  NC,  NC,
...
void winkbd_keydown(WPARAM wParam, LPARAM lParam) {
	...
	if (data != NC) {
		if ((data == 0x73) &&
				(xmiloscfg.keyboard == KEY_KEY101) &&
				(lParam & 0x01000000)) {
			data = 0x82;
		}
		else if ((xmiloscfg.keyboard != KEY_PC98) &&
				(!(lParam & 0x01000000)) &&
				(key106ext[wParam & 0xff] != NC)) {			// ver0.28
			keystat_keydown(0x70);							// PC/AT only!
			data = key106ext[wParam & 0xff];
		}
		keystat_keydown(data);
	}
	else {													// ver0.28
		if ((!xmiloscfg.keyboard != KEY_PC98) && (wParam == 0x0c)) {
			keystat_keydown(0x70);							// PC/AT only
			keystat_keydown(0x47);
		}
	}
}
